...
/*   Updated: 2025/06/19 14:27:18 by nfordoxc         ###   Luxembourg.lu     */
/*                                                                            */
/* ************************************************************************** */
...
MyCGI::MyCGI( Socket& socket )
	: _txBuffer(BUFF_SIZE), _rxBuffer(BUFF_SIZE), _socket(&socket)
{
...
void		MyCGI::setMap( void )
{
	...
	for(itMap = this->_env.begin(); itMap != this->_env.end(); ++itMap)
...
void		MyCGI::setParams( void )
{
	std::cout	<< "[DEBUUUUG]" << std::endl
				<< "PATH: " << this->_binaryExec << std::endl
				<< "SCRIPT: " << this->_scriptPath << std::endl
